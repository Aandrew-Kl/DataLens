...
export function setup() {
  const email = __ENV.TEST_EMAIL || `k6-auth-${Date.now()}@example.com`;
...
function getSessionToken(data) {
  if (!vuToken) {
    if (__VU <= 4) {
      vuToken = loginUser(data.email, data.password);
    } else {
      vuToken = data.token;
    }
  }
...
export default function (data) {
  ...
  const datasetName = `auth-${__VU}-${__ITER}`;
  const uploadedDataset = uploadDataset(token, datasetName);

  listDatasets(token, uploadedDataset.id);
  getDataset(token, uploadedDataset.id);
