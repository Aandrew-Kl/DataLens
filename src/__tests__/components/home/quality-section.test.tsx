...
jest.mock("@/components/data/data-quality-rules", () =>
  mockComponent("Data Quality Rules Panel"),
);
jest.mock("@/components/data/data-profiler-lite", () =>
  mockComponent("Data Profiler Lite"),
);
jest.mock("@/components/data/data-import-preview", () =>
  mockComponent("Data Import Preview"),
);
jest.mock("@/components/data/data-export-wizard", () =>
  mockComponent("Data Export Wizard"),
);
jest.mock("@/components/data/data-catalog-browser", () =>
  mockComponent("Data Catalog Browser"),
);
jest.mock("@/components/data/data-pipeline-builder", () =>
  mockComponent("Data Pipeline Builder"),
);
jest.mock("@/components/data/data-bookmark-manager", () =>
  mockComponent("Data Bookmark Manager"),
);
...
describe("QualitySection", () => {
  ...
  it("renders the section heading", () => {
    render(<QualitySection {...defaultProps} />);

    expect(
      screen.getByRole("heading", { level: 2, name: "Data Quality Rules" }),
    ).toBeInTheDocument();
  });

  it("renders the rules workspace", () => {
    render(<QualitySection {...defaultProps} />);

    expect(screen.getByText("Data Quality Rules Panel")).toBeInTheDocument();
  });
});
