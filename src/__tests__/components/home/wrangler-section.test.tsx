...
jest.mock("@/components/data/data-preview", () => mockComponent("Data Preview"));
jest.mock("@/components/data/data-summarizer", () =>
  mockComponent("Data Summarizer"),
);
jest.mock("@/components/data/data-wrangler", () =>
  mockComponent("Data Wrangler Panel"),
);
jest.mock("@/components/data/pivot-configurator", () =>
  mockComponent("Pivot Configurator Tool"),
);
jest.mock("@/components/data/row-editor", () => mockComponent("Row Editor"));
jest.mock("@/components/data/row-sampler-advanced", () =>
  mockComponent("Row Sampler Advanced"),
);
jest.mock("@/components/data/data-sampler-stratified", () =>
  mockComponent("Data Sampler Stratified"),
);
jest.mock("@/components/data/data-recipe-builder", () =>
  mockComponent("Data Recipe Builder"),
);
jest.mock("@/components/data/text-mining-tool", () =>
  mockComponent("Text Mining Tool"),
);
jest.mock("@/components/data/data-enrich-tool", () =>
  mockComponent("Data Enrich Tool"),
);
jest.mock("@/components/data/data-merge-tool", () =>
  mockComponent("Data Merge Tool"),
);
...
describe("WranglerSection", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it("renders the section heading", () => {
    render(<WranglerSection {...defaultProps} />);

    expect(
      screen.getByRole("heading", { level: 2, name: "Data Wrangler" }),
    ).toBeInTheDocument();
  });

  it("renders the wrangler workspace", () => {
    render(<WranglerSection {...defaultProps} />);

    expect(screen.getByText("Data Wrangler Panel")).toBeInTheDocument();
