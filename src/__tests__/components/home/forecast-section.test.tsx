...
jest.mock("@/components/data/data-forecast", () => mockComponent("Data Forecast"));
jest.mock("@/components/data/time-series-forecast", () =>
  mockComponent("Time Series Forecast"),
);
...
describe("ForecastSection", () => {
  ...
  it("renders the section heading", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "Forecasting" }),
    ).toBeInTheDocument();
  });

  it("renders the forecast workspace", () => {
    render(<ForecastSection {...defaultProps} />);

    expect(screen.getByText("Data Forecast")).toBeInTheDocument();
  });
});
