...
jest.mock("@/components/data/pivot-table", () =>
  mockComponent("Standard Pivot Table"),
);
jest.mock("@/components/data/pivot-table-advanced", () =>
  mockComponent("Advanced Pivot Table"),
);
...
describe("PivotSection", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it("renders the section heading", () => {
    render(<PivotSection {...defaultProps} />);

    expect(
      screen.getByRole("heading", { level: 2, name: "Pivot Table" }),
    ).toBeInTheDocument();
  });

  it("renders the default pivot table workspace", () => {
    render(<PivotSection {...defaultProps} />);

    expect(screen.getByText("Standard Pivot Table")).toBeInTheDocument();
    expect(screen.getByText("Pivot Configurator Tool")).toBeInTheDocument();
  });
});
