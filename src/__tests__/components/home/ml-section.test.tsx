...
jest.mock("@/components/ml/classification-view", () =>
  mockComponent("Classification View"),
);
jest.mock("@/components/ml/clustering-view", () =>
  mockComponent("Clustering View"),
);
jest.mock("@/components/ml/feature-importance", () =>
  mockComponent("Feature Importance"),
);
jest.mock("@/components/ml/regression-view", () =>
  mockComponent("Regression View"),
);
...
describe("MlSection", () => {
  ...
  it("renders the section heading", () => {
    render(<MlSection {...defaultProps} />);

    expect(
      screen.getByRole("heading", { level: 2, name: "Machine Learning" }),
    ).toBeInTheDocument();
  });

  it("renders the clustering workflow", () => {
    render(<MlSection {...defaultProps} />);

    expect(screen.getByText("Clustering View")).toBeInTheDocument();
  });
});
