...
jest.mock("@/components/ai/ai-playground", () =>
  mockComponent("AI Playground"),
);
jest.mock("@/components/ai/ai-schema-analyzer", () =>
  mockComponent("AI Schema Analyzer"),
);
jest.mock("@/components/query/chat-interface", () =>
  mockComponent("Chat Interface"),
);
jest.mock("@/components/query/natural-language-bar", () =>
  mockComponent("Natural Language Bar"),
);
...
const defaultProps = {
  ...
  tableName: "sales",
  columns,
  onAddNotification: jest.fn(() => "notification-1"),
};
...
describe("QuerySection", () => {
  ...
  it("renders the section heading", () => {
    render(<QuerySection {...defaultProps} />);

    expect(
      screen.getByRole("heading", { level: 2, name: "Ask Your Data" }),
    ).toBeInTheDocument();
  });

  it("renders the natural language query tools", () => {
    render(<QuerySection {...defaultProps} />);

    expect(screen.getByText("Natural Language Bar")).toBeInTheDocument();
