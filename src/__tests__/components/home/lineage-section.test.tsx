...
jest.mock("@/components/data/data-lineage-view", () =>
  mockComponent("Data Lineage View"),
);
jest.mock("@/components/data/schema-evolution", () =>
  mockComponent("Schema Evolution"),
);
jest.mock("@/components/data/data-snapshot-manager", () =>
  mockComponent("Data Snapshot Manager"),
);
...
describe("LineageSection", () => {
  ...
  it("renders the section heading", () => {
    render(<LineageSection {...defaultProps} />);

    expect(
      screen.getByRole("heading", { level: 2, name: "Data Lineage" }),
    ).toBeInTheDocument();
  });

  it("renders the lineage overview tools", () => {
    render(<LineageSection {...defaultProps} />);

    expect(screen.getByText("Data Lineage Graph")).toBeInTheDocument();
    expect(screen.getByText("Data Narrator")).toBeInTheDocument();
  });
});
