...
describe("ExploreSection", () => {
  ...
  it("renders the section heading", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "Explore Relationships" }),
    ).toBeInTheDocument();
  });

  it("renders the child tool components", () => {
    render(<ExploreSection {...defaultProps} />);

    expect(screen.getAllByText("Anomaly Heatmap").length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText("Correlation Finder").length).toBeGreaterThanOrEqual(1);
  });
});
