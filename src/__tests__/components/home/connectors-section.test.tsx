...
jest.mock("@/components/data/data-catalog", () => mockComponent("Data Catalog"));
jest.mock("@/components/data/data-connector", () =>
  mockComponent("Data Connector"),
);
jest.mock("@/components/data/merge-datasets", () =>
  mockComponent("Merge Datasets Tool"),
);
...
describe("ConnectorsSection", () => {
  ...
  it("renders the section heading", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "Data Connectors" }),
    ).toBeInTheDocument();
  });

  it("renders the connector workspace tools", () => {
    render(
      <ConnectorsSection
        onConnectorDataLoaded={jest.fn()}
        onRegisterMergedDataset={jest.fn()}
