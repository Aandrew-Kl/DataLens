...
describe("DataOpsPage", () => {
  ...
  it("renders the data operations heading", () => {
    render(<DataOpsPage />);
    ...
    expect(
      screen.getByText("Upload, manage and clean datasets before creating charts or dashboards."),
    ).toBeInTheDocument();
  });

  it("shows the dataset management UI for loaded datasets", () => {
    render(<DataOpsPage />);
  ...
  it("shows the upload section and file picker control", () => {
    render(<DataOpsPage />);

    expect(screen.getByRole("heading", { level: 2, name: "Dataset upload" })).toBeInTheDocument();
    expect(
      screen.getByText("Drag and drop CSV files here, or choose files from your device."),
    ).toBeInTheDocument();
