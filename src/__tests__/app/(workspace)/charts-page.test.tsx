...
import ChartsPage from "@/app/(workspace)/charts/page";
...
jest.mock("@/stores/chart-store", () => {
  const createState = () => ({
    savedCharts: [] as SavedChartConfig[],
    activeChartId: null as string | null,
    chartHistory: [] as SavedChartConfig[],
    addChart: jest.fn(),
    removeChart: jest.fn(),
    updateChart: jest.fn(),
...
describe("ChartsPage", () => {
  ...
  it("renders the heading, saved chart list, and create chart button", () => {
    mockedChartStoreModule.useChartStore.setMockState({
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "Chart preview" }),
    ).toBeInTheDocument();
  });

  it("shows the empty chart state when no charts exist", () => {
    mockedChartStoreModule.useChartStore.setMockState({
      savedCharts: [],
      activeChartId: null,
      chartHistory: [],
    ...
    expect(screen.getByText("No charts yet. Create one to get started.")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /new chart/i })).toBeInTheDocument();
  });
});
