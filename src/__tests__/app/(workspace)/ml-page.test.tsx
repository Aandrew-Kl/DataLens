...
import MLPage from "@/app/(workspace)/ml/page";
...
jest.mock("@/components/ml/clustering-view", () => ({
  __esModule: true,
  default: () => null,
}));

jest.mock("@/components/ml/regression-view", () => ({
  __esModule: true,
  default: () => null,
}));

jest.mock("@/components/ml/anomaly-detector", () => ({
  __esModule: true,
  default: () => null,
}));
...
describe("MLPage", () => {
  ...
  it("renders the machine learning heading", () => {
    render(<MLPage />);

    expect(
      screen.getByRole("heading", { level: 1, name: "Machine Learning" }),
    ).toBeInTheDocument();
    expect(screen.getByText(/analyze sales\.csv with interactive ml tools\./i)).toBeInTheDocument();
  });

  it("shows the ML tabs for regression, clustering, and outlier detection", () => {
    render(<MLPage />);

    expect(screen.getByRole("button", { name: /regression/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /clustering/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /outlier detection/i })).toBeInTheDocument();
  });

  it("shows the no-dataset fallback when no dataset is active", () => {
    mockedDatasetStoreModule.useDatasetStore.setMockState({
      datasets: [],
    ...
    expect(screen.getByText("No active dataset found")).toBeInTheDocument();
    expect(
      screen.getByText(
