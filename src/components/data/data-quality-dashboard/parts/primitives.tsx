...
export function ScorePill({ score }: { score: number }) {
  return (
    <div
      className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold tracking-[0.16em] uppercase ${getScoreTone(score)} bg-white dark:bg-slate-950`}
    >
      {formatPercent(score)}
    </div>
  );
