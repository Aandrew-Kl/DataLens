...
export default function DuplicateFinder({ tableName, columns }: DuplicateFinderProps) {
  ...
  useEffect(() => {
    const resetKey = `${tableName}::${columnSignature}`;
    ...
    <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, ease: "easeOut" }} className="overflow-hidden rounded-3xl border border-slate-200/70 bg-gradient-to-br from-white via-slate-50 to-sky-50/70 shadow-xl shadow-slate-950/5 dark:border-slate-800/80 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900">
      <div className="border-b border-slate-200/70 px-6 py-5 dark:border-slate-800/80">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-400/30 bg-sky-500/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-sky-700 dark:border-sky-500/30 dark:text-sky-300"><Search className="h-3.5 w-3.5" />Duplicate Row Finder</div>
            <h2 className="mt-3 text-xl font-semibold text-slate-950 dark:text-slate-50">Find duplicate groups in {tableName}</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-300">Pick the columns that define a duplicate key, then run a DuckDB `GROUP BY` query to surface repeated combinations and materialize a distinct key table.</p>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white/70 p-4 dark:border-slate-800/80 dark:bg-slate-950/35">
            ...
            <div className="flex flex-wrap gap-2 text-xs font-medium">
              <button type="button" onClick={() => setSelectedNames(columns.map((column) => column.name))} className="rounded-full border border-slate-300/80 px-3 py-1.5 text-slate-700 transition hover:border-sky-300 hover:text-sky-700 dark:border-slate-700 dark:text-slate-200 dark:hover:border-sky-500/50 dark:hover:text-sky-300">Select all</button>
              <button type="button" onClick={() => setSelectedNames([])} className="rounded-full border border-slate-300/80 px-3 py-1.5 text-slate-700 transition hover:border-slate-400 dark:border-slate-700 dark:text-slate-200 dark:hover:border-slate-600">Clear</button>
            </div>
          </div>
            ...
            {columns.map((column) => {
              ...
              return (
                <button key={column.name} type="button" onClick={() => toggleColumn(column.name)} className={`rounded-2xl border p-4 text-left transition ${active ? "border-sky-400/50 bg-sky-500/10 text-sky-900 dark:border-sky-500/40 dark:bg-sky-500/10 dark:text-sky-100" : "border-slate-200/80 bg-white/70 text-slate-700 hover:border-slate-300 dark:border-slate-800/80 dark:bg-slate-950/30 dark:text-slate-200 dark:hover:border-slate-700"}`}>
                  ...
                  <p className="mt-3 text-xs leading-5 text-slate-500 dark:text-slate-400">{formatNumber(column.uniqueCount)} unique values, {formatNumber(column.nullCount)} nulls.</p>
                </button>
              );
            })}
          </div>
          ...
          <AnimatePresence mode="wait">
            {status && (
              <motion.div key={status.message} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className={`rounded-2xl border px-4 py-3 text-sm ${status.kind === "success" ? "border-emerald-300/60 bg-emerald-500/10 text-emerald-800 dark:border-emerald-500/30 dark:text-emerald-300" : "border-red-300/60 bg-red-500/10 text-red-800 dark:border-red-500/30 dark:text-red-300"}`}>
                <div className="flex flex-wrap items-center gap-3">
                  <span className="inline-flex items-center gap-2 font-medium"><CheckCircle className="h-4 w-4" />{status.message}</span>
                  {createdTableName && status.kind === "success" && <button type="button" onClick={handleCopyTableName} className="inline-flex items-center gap-1 rounded-full border border-current/20 px-3 py-1 text-xs font-semibold"><Copy className="h-3.5 w-3.5" />{copied ? "Copied" : "Copy name"}</button>}
                </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="border-t border-slate-200/70 px-6 py-5 dark:border-slate-800/80">
        <div><h3 className="text-sm font-semibold text-slate-950 dark:text-slate-50">Sample duplicate groups</h3><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Showing up to {SAMPLE_LIMIT} repeated key combinations, ordered by duplicate count.</p></div>
        <AnimatePresence mode="wait">
          {loading ? (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center justify-center py-14 text-sm text-slate-500 dark:text-slate-400"><Loader2 className="mr-2 h-4 w-4 animate-spin text-sky-500" />Running duplicate query...</motion.div>
          ) : hasQueried && summary && summary.duplicateGroups === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="mt-4 rounded-2xl border border-emerald-300/50 bg-emerald-500/10 px-4 py-4 text-sm text-emerald-800 dark:border-emerald-500/30 dark:text-emerald-300">No duplicate groups were found for the currently selected columns.</motion.div>
          ) : sampleRows.length ? (
            ...
            <motion.div key="idle" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="mt-4 rounded-2xl border border-dashed border-slate-300/80 px-4 py-8 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">Choose one or more columns, then run the duplicate query to inspect repeated groups.</motion.div>
          )}
        </AnimatePresence>
      </div>
    </motion.section>
