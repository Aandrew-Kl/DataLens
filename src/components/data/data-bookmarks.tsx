...
export default function DataBookmarks({ tableName, columns }: DataBookmarksProps) {
  ...
  useEffect(() => {
    ...
    void bookmarksApi
      ...
      .then((remoteBookmarks) => {
        ...
        const mappedBookmarks = remoteBookmarks
          .filter(
            (bookmark): bookmark is typeof bookmark & {
              tableName: string;
              description: string | null;
              viewState: ViewState;
            } =>
              bookmark.tableName === tableName &&
              Boolean(bookmark.viewState) &&
              typeof bookmark.viewState === "object",
          )
          .map((bookmark) => ({
            id: bookmark.id,
            name: bookmark.label,
        ...
        const merged = new Map<string, ViewBookmark>();

        for (const bookmark of mappedBookmarks) {
  ...
  function deleteBookmark(bookmarkId: string) {
    ...
    if (authToken) {
      void bookmarksApi.delete(bookmarkId).catch(() => {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-white/20 bg-white shadow-2xl shadow-slate-950/10 dark:border-white/10 dark:bg-slate-950">
      ...
      <div className="space-y-5 px-5 py-5">
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: EASE }} className="grid gap-4 rounded-lg border border-white/15 bg-white p-4 dark:bg-slate-900 lg:grid-cols-[1.2fr_0.9fr]">
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="space-y-2">
                ...
                <select value={currentView.selectedTab} onChange={(event) => updateView({ ...currentView, selectedTab: event.target.value })} className="w-full rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50">
              ...
              <label className="space-y-2">
                ...
                <div className="grid grid-cols-[1fr_auto] gap-2">
                  <select value={currentView.sortColumn} onChange={(event) => updateView({ ...currentView, sortColumn: event.target.value })} className="rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50">
                    {columns.map((column) => <option key={column.name} value={column.name}>{column.name}</option>)}
                  </select>
                  <select value={currentView.sortDirection} onChange={(event) => updateView({ ...currentView, sortDirection: event.target.value as ViewState["sortDirection"] })} className="rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50">
            ...
            <div className="space-y-3 rounded-lg border border-white/15 bg-white p-4 dark:bg-slate-900">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Filters</h3>
                <button type="button" onClick={addFilter} className="inline-flex items-center gap-2 rounded-2xl border border-white/20 px-3 py-2 text-sm text-slate-700 transition hover:bg-white dark:text-slate-200">
              ...
              {currentView.filters.map((filter) => (
                <div key={filter.id} className="grid gap-2 sm:grid-cols-[1fr_auto_1fr_auto]">
                  <select value={filter.column} onChange={(event) => updateFilter(filter.id, { column: event.target.value })} className="rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50">
                    {columns.map((column) => <option key={column.name} value={column.name}>{column.name}</option>)}
                  </select>
                  <select value={filter.operator} onChange={(event) => updateFilter(filter.id, { operator: event.target.value as ViewFilter["operator"] })} className="rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50">
                    <option value="=">=</option>
                    <option value="!=">!=</option>
                  ...
                  <input value={filter.value} onChange={(event) => updateFilter(filter.id, { value: event.target.value })} placeholder="Value" className="rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50" />
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {columns.map((column) => (
                  <label key={column.name} className="flex items-center gap-3 rounded-2xl border border-white/15 bg-white px-3 py-2.5 text-sm text-slate-700 dark:bg-slate-900/35 dark:text-slate-200">
          ...
          <div className="space-y-3 rounded-lg border border-white/15 bg-slate-950/5 p-4 dark:bg-white/5">
            <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Bookmark name" className="w-full rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50" />
            <textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={4} placeholder="What makes this view useful?" className="w-full rounded-2xl border border-white/20 bg-white px-4 py-3 text-sm text-slate-950 outline-none dark:bg-slate-950 dark:text-slate-50" />
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <button type="button" onClick={exportBookmarks} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/20 px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-white dark:text-slate-200">
                <Download className="h-4 w-4" />
                Export JSON
              </button>
              <label htmlFor={`bookmark-import-${tableName}`} className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-2xl border border-white/20 px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-white dark:text-slate-200">
            ...
            <button type="button" onClick={() => updateView(defaultViewState(columns))} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/20 px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-white dark:text-slate-200">
          ...
          <AnimatePresence initial={false}>
            {bookmarks.length === 0 ? (
              ...
              bookmarks.map((bookmark) => (
                <motion.article key={bookmark.id} layout initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }} transition={{ duration: 0.25, ease: EASE }} className="rounded-lg border border-white/15 bg-white p-4 dark:bg-slate-900/35">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400">
                        <span className="rounded-full bg-white px-2.5 py-1 dark:bg-slate-950">{formatTimestamp(bookmark.timestamp)}</span>
                        <span className="rounded-full bg-white px-2.5 py-1 dark:bg-slate-950">{bookmark.state.selectedColumns.length} columns</span>
                        <span className="rounded-full bg-white px-2.5 py-1 dark:bg-slate-950">{bookmark.state.filters.length} filters</span>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button type="button" onClick={() => restoreBookmark(bookmark)} className="inline-flex items-center gap-2 rounded-2xl border border-white/20 px-3 py-2 text-sm text-slate-700 transition hover:bg-white dark:text-slate-200">
                        <RotateCcw className="h-4 w-4" />
                        Restore
                      </button>
                      <button type="button" onClick={() => startEditing(bookmark)} className="inline-flex items-center gap-2 rounded-2xl border border-white/20 px-3 py-2 text-sm text-slate-700 transition hover:bg-white dark:text-slate-200">
